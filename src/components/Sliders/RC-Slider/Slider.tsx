    ...
    <SliderContext.Provider value={context}>
      ...
      >
        <div className={`${prefixCls}-rail`} style={railStyle}>
        {
          center &&
            <S.CenterContainer 
              isRight={cacheValues[0] > ((max + min) / 2)}
              width={Math.abs(cacheValues[0]) / (max + Math.abs(min)) }
            >
              <S.Center width={Math.abs(cacheValues[0]) / (max + Math.abs(min)) }/>
            </S.CenterContainer>
        }
        </div>
...
const S = {
  CenterContainer: styled.div<{
    ...
    width: 100%;
    height: 100%;
    transform: ${props => props.isRight ? 'translateX(50%)' : `translateX(${50 - (props.width * 100)}%)`};
  `,
  Center: styled.div<{
    width: number
  }>`
